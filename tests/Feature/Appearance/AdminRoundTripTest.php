...
class AdminRoundTripTest extends TestCase
{
    ...
    public function test_the_posted_form_round_trips_through_the_settings_table(): void
    {
        ...
        $this->assertNotSame([], (array) $props['preview']['surface']['css_vars']);
    }
    ...
    public function test_switching_back_to_the_brand_surface_emits_no_css_variables(): void
    {
        ...
        $this->assertSame([], (array) $props['preview']['surface']['css_vars']);
    ...
    public function test_the_saved_appearance_reaches_the_login_page(): void
    {
        ...
        // Both sides through JSON: the meta tag has already been encoded and
        // decoded, while toInertiaProp() is raw PHP (css_vars is a stdClass so
        // `{}` survives encoding). Comparing the two shapes tests json_decode.
        $this->assertEquals(
            json_decode(json_encode(app(\App\Appearance\AppearanceManager::class)->toInertiaProp()), true),
            $emitted,
        );
