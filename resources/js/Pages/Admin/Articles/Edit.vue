...
import { ChevronDown, ImageIcon, X as XIcon } from 'lucide-vue-next';
...
const form = useForm({
    ...
    meta: {
        meta_title: props.article?.meta?.meta_title ?? '',
        meta_description: props.article?.meta?.meta_description ?? '',
        meta_keywords: props.article?.meta?.meta_keywords ?? '',
    },
...
const imagePreview = ref<string | null>(props.article?.featured_image_url ?? null);
...
function submit() {
    ...
    if (isEditing.value) {
        form
            ...
            .post(route('admin.articles.update', props.article!.id), options);
    } else {
        form
            .transform(() => formData)
...
<template>
    <AuthenticatedLayout :breadcrumbs="[
        ...
        { label: isEditing ? 'Edit Article' : 'Create Article' },
    ]">
        <Head :title="isEditing ? 'Edit Article' : 'Create Article'" />

        <PageHeader :title="isEditing ? 'Edit Article' : 'Create Article'" :description="isEditing ? `Editing: ${article!.title}` : 'Create a new blog article.'" />
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="title" v-model="form.title" placeholder="Article title" />
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="slug" v-model="form.slug" placeholder="article-slug" />
                    ...
                    <div class="space-y-2">
                        ...
                        <Textarea id="excerpt" v-model="form.excerpt" placeholder="Short summary of the article..." rows="3" />
